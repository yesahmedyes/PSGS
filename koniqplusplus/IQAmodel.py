...
class Model_Joint(nn.Module):
    def __init__(self, return_feature=False):
        ...
        features = list(
            models.__dict__["resnext101_32x8d"](pretrained=True).children()
        )[:-2]
...
class SideNet(nn.Module):
    def __init__(
        self,
        output=1,
        in_chns=[256, 256, 512, 512, 1024, 1024, 2048, 2048],
        out_chns=[64, 64, 64, 64, 128, 128, 256, 256],
    ):
        ...
        self.fusion_block1 = FFRM(
            in_channel=out_chns[0], out_channel=out_chns[1], pool=True
        )
        self.fusion_block2 = FFRM(
            in_channel=out_chns[1] + out_chns[2], out_channel=out_chns[3], pool=True
        )
        self.fusion_block3 = FFRM(
            in_channel=out_chns[3] + out_chns[4], out_channel=out_chns[5], pool=True
        )
        self.fusion_block4 = FFRM(
            in_channel=out_chns[5] + out_chns[6], out_channel=out_chns[7], pool=False
        )
...
def soft_pool2d_global(x, kernel_size=2, stride=None):
    ...
    return (
        F.adaptive_avg_pool2d(x.mul(e_x), 1)
        .mul_((h))
        .div_(F.adaptive_avg_pool2d(e_x, 1).mul_((w)))
    )
...
def soft_pool2d_local(x, kernel_size=2, stride=None, force_inplace=False):
    ...
    return (
        ...
        .mul_(kernel_size**2)
        .div_(F.avg_pool2d(e_x, kernel_size, stride=stride).mul_(kernel_size**2))
    )
